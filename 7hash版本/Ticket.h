...
struct Ticket{
	...
	Ticket(const sjtu::string &id,const sjtu::string &f,const sjtu::string &t,const Date &l,const Date &r,const int &cost_,const int &Num,const int tm=0):
...
void TicketManagement::insert_ticket(const sjtu::string &username_,const int &timestamp,const Ticket &t,const bool flag){
	...
	if(tmp.time==2)inqu_ticket.insert( sjtu::pair<type1,type2>(type1(H(t.trainID.change()),timestamp),type2(username_,tmp)) );
}
