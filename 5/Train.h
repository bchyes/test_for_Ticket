...
public:
	...
	Stations(const sjtu::string *data,const int &k,const char &Ty):sz(k),type(Ty){
...
private:
	...
	sjtu::bpt<sjtu::string,bool> rel;
	sjtu::bpt<sjtu::string,Stations> sta;
	sjtu::bpt<sjtu::string,Times> Ti;
	sjtu::bpt<sjtu::string,Prices> pri;
...
TrainManagement::TrainManagement(): Road("file_Road.dat","file_Road_delete.dat"),
...
void TrainManagement::add_train(const std::string &trainID_,const int &stationNum_,const sjtu::string *stations_,const int &seatNum,const int *prices_,const int *travelTimes_,
	...
	Tk.insert(sjtu::pair<sjtu::string,Left_Ticket>(trainID,Left_Ticket(saleDateR_-saleDateL_,stationNum_,seatNum)));
	rel.insert(sjtu::pair<sjtu::string,bool>(trainID,0));
	sta.insert(sjtu::pair<sjtu::string,Stations>(trainID,Stations(stations_,stationNum_,type_)));
	Date L(saleDateL_.m,saleDateL_.d,startTime_.hr,startTime_.mi),R(saleDateR_.m,saleDateR_.d,startTime_.hr,startTime_.mi);
	Ti.insert(sjtu::pair<sjtu::string,Times>(trainID,Times(travelTimes_,stopoverTimes_,L,R,stationNum_)));
	pri.insert(sjtu::pair<sjtu::string,Prices>(trainID,Prices(prices_,stationNum_)));
	puts("0");
}
...
Ticket TrainManagement::get_ticket(const sjtu::string &trainID,const Date &date_,const sjtu::string From,const sjtu::string To,const int &ask){
	if(rel.empty() || !rel.count(trainID) || rel.find(trainID)!=1)return Ticket("");
	...
	if(tk.maxSeat<ask)return Ticket("");
	Ticket ret;
	...
	ret.To=To;
	ret.TimeL=date;
	ret.num=tk.query_ticket(date-L,l,r);
	...
	ret.TimeR=date; 
	ret.cost=pri.find(trainID).Ask(l,r);
	return ret;
}

void TrainManagement::Update_ticket(const Ticket &t){
	Left_Ticket now=Tk.find(t.trainID);
	int l=sta.find(t.trainID).find(t.From),r=sta.find(t.trainID).find(t.To);
	now.Update_ticket(t.TimeL-Ti.find(t.trainID).Ask(l),l,r,t.num); 
...
void TrainManagement::query_ticket(const std::string &From,const std::string &To,const Date &date,bool flag){
	...
	if(Road.empty() || !Road.count(road))return void(puts("0"));
...
void TrainManagement::query_transfer(const std::string &From_,const std::string &To_,const Date &date,bool flag){
	...
	sjtu::string last(""),now=rel.upper_bound(last);

	while(last!=now){
		last=now;
		now=rel.upper_bound(last);
